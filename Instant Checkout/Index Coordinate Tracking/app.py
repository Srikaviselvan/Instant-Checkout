...
def start_tkinter():
    global field, closest_object_field

    window = Tk()

    Label(window, text = "All Objects Detected").pack()
    field = Text(window, width=40, height=10)
    field.pack()
    Label(window, text = "Hand is closest to ").pack()
    closest_object_field = Entry(window)
    closest_object_field.pack()
...
def main():
    ...
    while True:
        global string_to_display, field
        fps = cvFpsCalc.get()
        ...
        detections = object_detection_model(image)[0]
        ...
        for detection in detections.boxes.data.tolist():
            ...
            if detection[-1] != 0:
                all_x_coordinates.append([detection[-1], ((detection[0] + detection[2]) / 2, (detection[1] + detection[3]) / 2)])
        

        global get_all_coordinates
        ...
        global valid_ids
        valid_ids = [39, 45, 67, 76]

        # Define a list of corresponding object names for the valid IDs
        global corresponding_objects
        corresponding_objects = ['Water Bottle', 'Bowl', 'Cell Phone', 'Scissors']
        ...
        for i in range(len(objects)):
            ...
            query = """INSERT INTO stock (object_id, object_name, object_quantity) VALUES (%s, %s, %s)"""
            data = (valid_ids[corresponding_objects.index(objects[i])], objects[i], object_count[i])
        ...
        for detection in detections.boxes.data.tolist():
            if int(detection[-1]) in valid_ids:
                cv.rectangle(image, (int(detection[0]), int(detection[1])), (int(detection[2]), int(detection[3])), (0, 255, 0), 2)
                cv.putText(image, corresponding_objects[valid_ids.index(int(detection[-1]))], (int(detection[0]), int(detection[1]) - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5, (255,0,0), 1)
...
# Defining function to calculate distance between index finger and objects detected
def calculate_distance(x, y, all_name_point):

    if len(all_name_point) > 0:

        temp_name_point = all_name_point.copy()

        all_name_point = []

        for i in range(len(temp_name_point)):
            if int(temp_name_point[i][0]) in valid_ids:
                all_name_point.append(temp_name_point[i])
        
        if len(all_name_point) == 0:
            return False

        # Initializing minimum_index and minimum_distance to first object
        minimum_index = 0
        minimum_distance = (x - all_name_point[minimum_index][1][0]) ** 2 + (y - all_name_point[minimum_index][1][1]) ** 2
        
        # Finding minimum_index and minimum_distance
        for index in range(len(all_name_point)):
            distance = (x - all_name_point[index][1][0]) ** 2 + (y - all_name_point[index][1][1]) ** 2
            if distance < minimum_distance:
                minimum_index = index
                minimum_distance = distance


        # Return name of Object Closest to Index finger
        return corresponding_objects[valid_ids.index(all_name_point[minimum_index][0])]


# Defining function to calculate for Bounding Boxes of Hand
...
# Defining function to calculate landmarks
...
# Defining function to draw landmarks on Hand
def draw_landmarks(image, landmark_point):
    ...
    # Clear field incase object is not detected
    def throwaway():
        sleep(3)
        closest_object_field.delete(0, END)

    x = landmark_point[8][0]
    y = landmark_point[8][1]

    all_coordinates = get_all_coordinates()
    closest_object = calculate_distance(x, y, all_coordinates)
    if closest_object:
        closest_object_field.delete(0, END)
        closest_object_field.insert(0, closest_object)
        Thread(target = throwaway).start()
    else:
        closest_object_field.delete(0, END)
